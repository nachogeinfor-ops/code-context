...
| `CC_SYMBOL_RERANK` | `off` | Opt-in cross-encoder rerank on `find_references`. `on`/`true`/`1` enables it; over-fetches 3x `max_count` from the symbol index and re-orders by `(symbol_name, snippet)` cross-encoder relevance. Reuses the `CC_RERANK` reranker when both are on, otherwise builds a fresh one. Adds ~0.5-1s latency on CPU; ship opt-in until eval validates a clean win. **Since v1.13.0.** |
...
| `CC_EMBED_BATCH_SIZE` | (unset) | Optional cap on the sentence-transformers `encode()` batch size used by `LocalST.embed()`. Default unset = sentence-transformers' built-in default (32). Positive int = explicit cap; non-positive (0, negative) coerced to unset. Lower values trade encode throughput for memory pressure — useful for large-context models (e.g. `nomic-ai/CodeRankEmbed` at 8192 max_seq) on memory-constrained hosts. **Note**: Sprint 15.1 confirmed that batch caps alone do NOT resolve the nomic hybrid-mode hang on large code-heavy repos (the stall fingerprint is identical at batch=4 and batch=32). Knob exposed as a general control, not as a fix. **Since v1.9.3.** |
| `CC_EMBED_MAX_CHARS` | `2048` | Per-chunk character cap before embedding. Default 2048 chars ≈ 512 tokens for code, matching BERT-family context windows. Long chunks are truncated to the head; the full snippet is preserved in the search response. Sprint 15.1 found that lowering this to **512** is the workaround that unlocks `nomic-ai/CodeRankEmbed` in hybrid mode on large code-heavy repos (the previous hang at 2048 was sequence-length-driven: the attention matrix over long inputs hit a pathological code path in NomicBert's custom forward). At 512, nomic hybrid on the 305-file C# WinServiceScheduler fixture completes cleanly in ~52 min with **NDCG@10 0.6249 (+0.218 vs MiniLM)**. Non-positive or non-integer values coerce to the default. **Since v1.9.4.** |
...
| `jinaai/jina-embeddings-v2-base-code` | ~640 MB | 768 | Code (functions, identifiers) | Apache-2.0. Trained on GitHub + 150M code+docstring pairs. **Requires `CC_TRUST_REMOTE_CODE=true`** because the model uses a custom JinaBert architecture. **v1.9.1** ships a compat shim that backports the v4-era APIs jina's custom code needs (`find_pruneable_heads_and_indices`, four `PretrainedConfig` defaults) — sufficient on `transformers >=4.49 and <5`. On `transformers >=5` the shim is not enough: jina's custom code also calls `PreTrainedModel.get_head_mask` which was removed in v5. Recommended on `transformers >=5`: pin `transformers<5` OR pick `nomic-ai/CodeRankEmbed` / `BAAI/bge-base-en-v1.5`. |
| `nomic-ai/CodeRankEmbed` | ~520 MB | 768 | Code (top pick for C# / Razor in Sprint 15 eval — small/medium repos only) | MIT. NomicBert architecture, 8192 max_seq. **Requires `CC_TRUST_REMOTE_CODE=on` + `pip install einops`.** Sprint 15 eval (vector_only) showed a massive lift on C# (NDCG +0.245 vs MiniLM, hit@1 doubled), small regression on the small Python fixture, neutral on TypeScript. **Workaround for large repos (Sprint 15.1, v1.9.4):** the original v1.9.0 hang in hybrid mode on the 305-file C# WinServiceScheduler fixture is caused by long-sequence attention compute, not threading or batch size. Setting `CC_EMBED_MAX_CHARS=512` (down from default 2048) resolves it: C# hybrid completes in ~52 min wall clock with NDCG@10 0.6249 (+0.218 vs MiniLM baseline 0.4065). **Note**: hybrid_rerank regresses (0.4028, -0.030 vs MiniLM rerank) because the default English-prose cross-encoder undermines nomic's code-tuned ranking. Recommend `CC_RERANK=off` when using nomic on code-heavy repos. Hypotheses ruled out before this: D — threading (`OMP_NUM_THREADS=4`, v1.9.2); A — batch size (`CC_EMBED_BATCH_SIZE=4`, v1.9.3). Small repos work at the default 2048. **Since v1.9.0.** |
