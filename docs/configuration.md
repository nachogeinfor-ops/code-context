...
| `nomic-ai/CodeRankEmbed` | ~520 MB | 768 | Code (top pick for C# / Razor in Sprint 15 eval — small/medium repos only) | MIT. NomicBert architecture, 8192 max_seq. **Requires `CC_TRUST_REMOTE_CODE=on` + `pip install einops`.** Sprint 15 eval (vector_only) showed a massive lift on C# (NDCG +0.245 vs MiniLM, hit@1 doubled), small regression on the small Python fixture, neutral on TypeScript. **Known issue (Sprint 15.1, v1.9.2 / v1.9.3):** in hybrid mode (`CC_KEYWORD_INDEX=sqlite`), nomic stalls during the cold reindex on the 305-file C# WinServiceScheduler fixture (Windows + CPU). Hybrid mode works fine on small repos (python_repo 16 files NDCG 0.7993; ts_repo 20 files NDCG 0.7776, both completed in ~10 min). The stall fingerprint (151 MB/s memory-mapped reads + ~0 disk writes for 30+ min) is identical with `OMP_NUM_THREADS=4` (v1.9.2 ruled threading out) AND with `CC_EMBED_BATCH_SIZE=4` (v1.9.3 ruled batch size out). Likely a NomicBert custom-code path triggered by high chunk counts. Vector-only mode (`CC_KEYWORD_INDEX=none`) works on large repos. **Since v1.9.0.** |
